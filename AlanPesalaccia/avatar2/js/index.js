...
function iniciarJuego() {
  ...
  // Escucha el cambio de selección en los inputs de radio
  document.querySelectorAll('input[name="personaje"]').forEach((radio) => {
    radio.addEventListener("change", function () {
      // Almacena el personaje seleccionado
      personajeSeleccionado = this.getAttribute("data-personaje");

      // Remueve la clase seleccionada de todos los labels
      document.querySelectorAll("label").forEach((label) => {
        label.classList.remove("seleccionado");
      });

      // Agrega la clase seleccionada al label correspondiente
      let labelSeleccionado = document.querySelector(
        `label[for="${this.id}"]`
      );
      labelSeleccionado.classList.add("seleccionado");
